...
function makeCollectResult(): CollectResult {
  return { structure: emptyStructure, prior: emptyPrior, rawPages: [], edges: [] }
}
...
describe('runRun', () => {
  ...
  it('saves the baseline in run (no longer a report concern)', async () => {
    ...
    const deps = {
      collect: vi.fn().mockResolvedValue({ structure, prior: { baseline: null, latestReport: null, feedback: [] }, rawPages: [], edges: [] }),
      detectDiffs: vi.fn().mockResolvedValue([]),
      runVerify: vi.fn().mockResolvedValue([]),
      writeFindings: vi.fn().mockResolvedValue(undefined),
...
describe('runRun — explore integration (run --explore)', () => {
  ...
  it('diff uses the clean pre-explore structure, not the post-explore re-crawl', async () => {
    ...
    const deps = {
      collect: vi.fn().mockResolvedValue({ structure: cleanStructure, prior: emptyPrior, rawPages: [], edges: [] }),
