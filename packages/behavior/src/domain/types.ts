...
export type TargetEnv = {
  ...
  auth?: {
    ...
    loginPath?: string
    /** Absolute login-form URL (cross-domain logins); takes precedence over loginPath. */
    loginUrl?: string
    username?: string
    password?: string
  }
...
export type VerifyFinding = {
  category: 'layout' | 'security' | 'access-control' | 'conditional' | 'registered-data' | 'error-handling' | 'login' | 'scenario' | 'input-validation' | 'crud'
  severity: 'high' | 'medium' | 'low'
  title: string
  detail: string
  evidence: string
...
export type Report = {
  ...
  summary: string
  /** relative path to this run's recorded API transactions jsonl (if any). */
