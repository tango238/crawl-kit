...
describe('pipeline/collect', () => {
  ...
  it('throws a clear error when config.targets is empty, instead of a bare TypeError', async () => {
    const { collect } = await import('./collect.js')
    const mockStore = makeMockStore(null)
    const mockCrawl = makeMockCrawl()
    const mockExtract = makeMockExtract()
    ...
    const ctx: RunContext = {
      root,
      runId: 'run-009',
      config: { ...makeConfig(), targets: [] },
      secrets: { db: {}, targetAuth: {}, anthropicApiKey: 'key', githubToken: 'tok' },
    }

    await expect(
      collect(ctx, { store: mockStore, crawl: mockCrawl, extractPageInfo: mockExtract, browser: makeFakeBrowser() }),
    ).rejects.toThrow(/targets が未設定です/)

    // Guard fires before any crawl/extract work happens
    expect(mockCrawl).not.toHaveBeenCalled()
    expect(mockExtract).not.toHaveBeenCalled()
  })

  it('populates CollectResult.edges from edges reported by crawl via onEdge, and persists behavior.edges.json', async () => {
    const { collect } = await import('./collect.js')
    const rawPages = [makeRawPage(), { ...makeRawPage(), url: 'https://example.com/about', title: 'About' }]
    const edge: NavEdge = { from: 'https://example.com', to: 'https://example.com/about', kind: 'link' }
    // Fake crawl that reports one edge via the onEdge callback it's given, mirroring how the real
    // crawl forwards discoverPages' onEdge.
    const mockCrawl = vi.fn(async (_b: BrowserLike, _t: unknown, _s: unknown, _d: string, onEdge?: (e: NavEdge) => void) => {
      onEdge?.(edge)
      return rawPages
    })
    const mockStore = makeMockStore(null)
    ...
    const persisted = JSON.parse(await readFile(join(root, 'data', 'behavior.edges.json'), 'utf8'))
    expect(persisted).toEqual({ runId: 'run-007', edges: [edge] })
  })

  it('collect→edges→sitemap integration: buildSitemap consumes CollectResult.edges + page urls directly', async () => {
    const { collect } = await import('./collect.js')
    const { buildSitemap } = await import('../sitemap.js')
    const rawPages = [makeRawPage(), { ...makeRawPage(), url: 'https://example.com/about', title: 'About' }]
    const edge: NavEdge = { from: 'https://example.com/', to: 'https://example.com/about', kind: 'link' }
    // Fake crawl reports one edge via onEdge, same as the real crawler forwards discoverPages' onEdge.
    const mockCrawl = vi.fn(async (_b: BrowserLike, _t: unknown, _s: unknown, _d: string, onEdge?: (e: NavEdge) => void) => {
      onEdge?.(edge)
      return rawPages
    })
    const mockStore = makeMockStore(null)
    // Echo url/title per raw page (rather than the shared makePageInfo() fixture) so the two
    // crawled pages don't collapse into one PageInfo — buildSitemap needs distinct page urls.
    const mockExtract = vi.fn(async (_llm: unknown, raw: RawPage) => ({ ...makePageInfo(), url: raw.url, title: raw.title }))
    ...
    // Task 1's CollectResult.edges (NavEdge[]) feeds straight into Task 2's buildSitemap — no
    // adapter/translation needed, proving the shared NavEdge shape actually lines up end to end.
    const pageUrls = result.structure.pages.map((p) => p.url)
    const sitemap = buildSitemap(result.edges, pageUrls, { now: '2026-07-03T00:00:00.000Z' })
  ...
  it('does not touch behavior.edges.json when browser is null (skip crawl), preserving a prior run\'s edges', async () => {
    const { collect } = await import('./collect.js')
    const { writeJsonAtomic, DATA_FILES, dataPath } = await import('@crawl-kit/contract')
    const existing = { runId: 'prior-run', edges: [{ from: 'https://example.com/', to: 'https://example.com/about', kind: 'link' }] }
    await writeJsonAtomic(dataPath(DATA_FILES.behaviorEdges, root), existing)
    ...
    expect(mockCrawl).not.toHaveBeenCalled()
    // The pre-existing edges.json (from a real, browser-backed run) must survive untouched —
    // writing `{ runId: 'run-011', edges: [] }` here would silently clobber it.
    const persisted = JSON.parse(await readFile(dataPath(DATA_FILES.behaviorEdges, root), 'utf8'))
    expect(persisted).toEqual(existing)
  })

  it('returns an empty edges array (and still persists) when crawl never calls onEdge', async () => {
    const { collect } = await import('./collect.js')
    const mockStore = makeMockStore(null)
    const mockCrawl = makeMockCrawl()
    const mockExtract = makeMockExtract()
    ...
    expect(result.edges).toEqual([])
    const persisted = JSON.parse(await readFile(join(root, 'data', 'behavior.edges.json'), 'utf8'))
    expect(persisted).toEqual({ runId: 'run-008', edges: [] })
  })
})
