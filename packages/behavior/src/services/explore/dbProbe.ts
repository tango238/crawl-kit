...
/**
 * True if NO row exists in `table` where `column` = `value` (i.e. a deletion is confirmed).
 * Parameterized. Never throws — returns false (does NOT assert absence) on error or a
 * non-identifier table/column, so a failed probe never falsely reports "deleted".
 */
export async function wasValueAbsent(
  ...
  if (!IDENT.test(table) || !IDENT.test(column)) {
    logger.warn({ table, column }, 'wasValueAbsent: non-identifier table/column — refusing to query')
    return false
  }
  const placeholder = dbType === 'postgres' ? '$1' : '?'
  const sql = `SELECT 1 FROM ${table} WHERE ${column} = ${placeholder} LIMIT 1`
