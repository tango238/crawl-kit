...
import { z } from 'zod'
import { makeLimiter, resolveClaudeCodeConcurrency } from '@crawl-kit/contract'
...
export type CreateLlmOptions = {
  ...
  claudeCode?: {
    ...
    timeoutMs?: number
    /**
     * Max `claude` subprocesses running at once. Each costs hundreds of MB, so this caps
     * memory regardless of how many LLM calls the pipeline fans out. Falls back to the
     * CLAUDE_CODE_MAX_CONCURRENCY env var, then to DEFAULT_CLI_MAX_CONCURRENCY (3).
     */
...
export function createLlm(
  ...
  // One shared limiter gates BOTH backends by the same CLAUDE_CODE_MAX_CONCURRENCY env.
  // For claude-code it bounds live `claude` subprocesses (memory); for the API it bounds
  // concurrent requests (rate limits). Same fan-out, same cap, one knob.
  const limit = makeLimiter(resolveClaudeCodeConcurrency(options.claudeCode?.maxConcurrency))
  const backend: LlmBackend =
    provider === 'claude-code'
      ? makeClaudeCodeBackend(options)
      : makeApiBackend(apiKey, options)
  const generate: LlmBackend = (model, fullPrompt) => limit(() => backend(model, fullPrompt))
...
function makeClaudeCodeBackend(options: CreateLlmOptions): LlmBackend {
  ...
  // Concurrency is bounded by the shared limiter in createLlm (covers both backends).
  return async (model, fullPrompt) =>
    parseClaudeCliOutput(await runner({ bin, model, prompt: fullPrompt, timeoutMs }))
}
