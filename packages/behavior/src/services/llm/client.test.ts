...
describe('API backend concurrency cap', () => {
  it('never runs more than maxConcurrency API calls at once', async () => {
    let active = 0
    let peak = 0
    ...
    const llm = createLlm('', models, {
      client: { messages: { create } } as unknown as CreateLlmOptions['client'],
      claudeCode: { maxConcurrency: 2 },
    })

    const calls = Promise.all(Array.from({ length: 6 }, () => llm.complete('report', 'p')))
    await new Promise((r) => setTimeout(r, 0))
    expect(release.length).toBe(2) // only 2 in flight, 4 queued
