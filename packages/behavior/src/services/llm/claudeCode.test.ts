...
describe('claude-code concurrency cap', () => {
  const origEnv = process.env['CLAUDE_CODE_MAX_CONCURRENCY']

  afterEach(() => {
    if (origEnv === undefined) delete process.env['CLAUDE_CODE_MAX_CONCURRENCY']
    else process.env['CLAUDE_CODE_MAX_CONCURRENCY'] = origEnv
  })

  /** A runner that blocks until released, so we can observe how many spawn at once. */
  const makeBlockingRunner = () => {
    let active = 0
    let peak = 0
    ...
    return { runner, releaseNext: () => release.shift()?.(), peak: () => peak, pending: () => release.length }
  }

  it('never spawns more than maxConcurrency runners at once', async () => {
    const { runner, releaseNext, peak, pending } = makeBlockingRunner()
    const llm = createLlm('', models, {
      provider: 'claude-code',
      claudeCode: { runner, maxConcurrency: 2 },
    })

    const calls = Promise.all(Array.from({ length: 6 }, () => llm.complete('verification', 'p')))
    await new Promise((r) => setTimeout(r, 0))
    expect(pending()).toBe(2) // only 2 in flight, 4 queued
  ...
  it('reads the cap from CLAUDE_CODE_MAX_CONCURRENCY when no option is given', async () => {
    process.env['CLAUDE_CODE_MAX_CONCURRENCY'] = '1'
    const { runner, releaseNext, peak, pending } = makeBlockingRunner()
    const llm = createLlm('', models, { provider: 'claude-code', claudeCode: { runner } })

    const calls = Promise.all(Array.from({ length: 3 }, () => llm.complete('verification', 'p')))
    await new Promise((r) => setTimeout(r, 0))
