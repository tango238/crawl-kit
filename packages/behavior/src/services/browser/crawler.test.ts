...
describe('crawler (unit, fake browser)', () => {
  ...
  it('merges BFS discovery pages beyond base + scenario when discover is set', async () => {
    const { crawlWithBrowser } = await import('./crawler.js')

    let currentUrl = 'https://example.com/'
    const rootHtml = '<html><body><a href="/a">A</a><a href="/b">B</a></body></html>'
    const page = makeFakePage({
      goto: vi.fn().mockImplementation(async (url: string) => { currentUrl = url }),
      url: vi.fn().mockImplementation(() => currentUrl),
      // Only the discovery root (…/) exposes links; leaf pages have none.
    ...
    expect(pages.length).toBeGreaterThanOrEqual(3) // base + /a + /b
    expect(urls).toContain('https://example.com/a')
    expect(urls).toContain('https://example.com/b')
  })

  it('caps merged pages at discover.maxPages', async () => {
    const { crawlWithBrowser } = await import('./crawler.js')

    let currentUrl = 'https://example.com/'
    const rootHtml = '<html><body><a href="/a">A</a><a href="/b">B</a><a href="/c">C</a></body></html>'
    const page = makeFakePage({
      goto: vi.fn().mockImplementation(async (url: string) => { currentUrl = url }),
      url: vi.fn().mockImplementation(() => currentUrl),
      content: vi.fn().mockImplementation(async () =>
    ...
    const pages = await crawlWithBrowser(
      ...
      [],
      '/tmp',
      { discover: { maxPages: 2, maxDepth: 2, excludePaths: [] } },
    )
