...
export async function executeLoginScenario(
  ...
  // An explicit absolute loginUrl (cross-domain login form) wins; otherwise baseUrl + loginPath.
  const loginUrl = target.auth?.loginUrl ?? `${target.baseUrl.replace(/\/$/, '')}${target.auth?.loginPath ?? '/login'}`
  // Path used for the "moved away from the login form" success check.
  const loginPath = target.auth?.loginUrl ? new URL(target.auth.loginUrl).pathname : (target.auth?.loginPath ?? '/login')
  ...
  // Advance when the URL leaves the login path OR lands on a 2FA page (which may live UNDER
  // the login path, e.g. /signin/two-factor-auth).
  await waitForUrl(page, (u) => !urlMatchesPath(u, loginPath) || looksLikeTwoFactorPage(u), navTimeoutMs, sleep)

  const afterSubmitUrl = page.url()

  // 2FA can live UNDER the login path (e.g. /administrator/signin/two-factor-auth), so handle it
  // BEFORE the "still on login path" failure verdict — the prefix match would otherwise misfire.
  // If 2FA is configured but the app went straight to the dashboard (remembered device / no 2FA),
  // the PIN step is skipped and login is treated as already succeeded. 2FA config is owned by the
  // scenario (env-specific); fall back to deps.twoFactor for authenticate()'s synthetic scenario.
